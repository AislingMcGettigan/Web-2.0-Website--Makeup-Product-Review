...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <style>
        ...
        <div class="center">
          <p>
            ...
            <b>Tag Filter:</b>
            <asp:DropDownList ID="ddlTagFilter" runat="server" CssClass="form-control centerDD" Width="30%" AutoPostBack="true" DataSourceID="SqlDataSource2" DataTextField="Tag" DataValueField="Tag" AppendDataBoundItems="true">

                <asp:ListItem Text="Select a Tag" Value="%">All</asp:ListItem>
            </asp:DropDownList>
        </div>

        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:dbam17ahnConnectionString %>" SelectCommand="SELECT DISTINCT tg.[Id], tg.[Tag] FROM [TagsJunction] tj LEFT OUTER JOIN [Tags] tg ON tg.ID = tj.FK_TagId"></asp:SqlDataSource>
    ...
    <p>
      <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:dbam17ahnConnectionString %>" SelectCommand="SELECT DISTINCT pr.Id, pr.ProductName, img.ImagePath, prt.ProductType, brn.BrandName, AverageRating FROM ViewProductsAndRatings AS pr LEFT OUTER JOIN Image AS img ON pr.ImageId = img.Id LEFT OUTER JOIN ProductType AS prt ON pr.ProductTypeId = prt.Id LEFT OUTER JOIN BrandTable AS brn ON pr.BrandId = brn.Id LEFT OUTER JOIN TagsJunction AS tj ON tj.FK_ProductId = pr.Id LEFT OUTER JOIN Tags AS tg ON tg.Id = tj.FK_TagId WHERE tg.Tag LIKE @tagFilter">
          <SelectParameters>
                <asp:ControlParameter ControlID="ddlTagFilter" Name="tagFilter" PropertyName="SelectedValue" />
            </SelectParameters>
       </asp:SqlDataSource>
</p>
    <p>
